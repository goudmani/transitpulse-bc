...
<!-- agent:status:begin -->
**As of 2026-08-18, the data half is in production and the model half is not.**
...
Nothing is trained yet, so no model has been registered and no endpoint exists. Phase 5 splits train/validation/test by time, which needs 21 distinct service days; **7 are collected**, 14 to go.

Gross usage on 2026-08-17 was **$0.84**, a $0.84/day median over the last three days (≈$26/month at that rate).
<!-- agent:status:end -->
...
Over 3,509,532 labelled stop arrivals, 2026-08-11 to 2026-08-17. **Preliminary**: too few days to cover a weekend, rain, or an incident. Figures come from `sql/07_profile_queries.sql`.

| Predictor | MAE (seconds) |
|---|---|
| Published schedule (predict zero delay) | **166.2** |
| Persistence (bus stays as late as it currently is) | **144.5** |
| Historical median for route/stop/hour | see `sql/07_profile_queries.sql` query 4 |
| **XGBoost model** | pending, Phase 6 |

Persistence beats the printed timetable by 13.1%. The registry gate is `mae_ratio_vs_persistence <= 0.92`, so a model must reach **≤ 132.9 seconds** to be registered at all.
<!-- agent:baselines:end -->
...
3,554,281 stop arrivals over 7 days of collection (2026-08-11 to 2026-08-17), label completeness 0.987.
